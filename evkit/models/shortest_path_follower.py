# From https://github.com/facebookresearch/habitat-api/blob/master/habitat/tasks/nav/shortest_path_follower.py
...
class ShortestPathFollower:
    ...
    def get_next_action(
        ...
        if (
            self._geo_dist(goal_pos)
            # np.linalg.norm(goal_pos - self._sim.get_agent_state().position)  # use geodesic distance to determine how far we are
            <= self._goal_radius
        ):
