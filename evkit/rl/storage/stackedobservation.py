...
class StackedSensorDictStorage(object):
    ...
    def __init__(self, n_rollouts, n_stack, sensor_obs_shapes):
        ...
        '''
        assert n_rollouts != 0, 'Cannot make a storage for 0 rollouts'
        assert n_stack != 0, 'Cannot stack 0 frames'
    ...
    def cuda(self, device=None):
        for sensor_name in self.sensor_names:
            self[sensor_name].cuda(device=device)
        return self
...
class StackedTensorStorage(object):
    ...
    def insert(self, obs, mask_out_done=None):
        ...
        if isinstance(obs, np.ndarray):
            ...
            # obs = torch.stack(obs, dim=0)  # This does not work
    ...
    def cuda(self, device=None):
        self.obs = self.obs.cuda(device=device)
        return self
