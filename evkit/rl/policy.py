...
class PolicyWithBase(BasePolicy):
    def __init__(self, base, action_space, num_stacks=4, loss_kwargs={}, gpu_devices=None,
                 base_class=None, base_kwargs={},
                 takeover:TakeoverPolicy = None, validator:ActionValidator = None):
        '''
            Args:
        ...
        if base is None:
            self.base = eval(base_class)(**base_kwargs)
        else:
            self.base = base
        ...
        if 'decoder_path' in self.loss_kwargs:
            task = [t for t in SINGLE_IMAGE_TASKS if t in self.loss_kwargs['decoder_path']][0]
            decoder = TaskonomyDecoder(out_channels=TASKS_TO_CHANNELS[task], eval_only=True)
            checkpoint = torch.load(self.loss_kwargs['decoder_path'])
            decoder.load_state_dict(checkpoint['state_dict'])
            decoder.cuda()
            decoder.eval()
            self.loss_kwargs['decoder'] = decoder
            print(f"Loaded TaskonomyDecoder from {self.loss_kwargs['decoder_path']}")



    def forward(self, observations, internal_states=None, masks=None, cache={}):
        ''' TODO: find out what these do '''
        value, actor_features, states = self.base(observations, internal_states, masks, cache=cache)
        dist = self.dist(actor_features)
        return dist.logits
    ...
    def act(self, observations, model_states, masks, deterministic=False):
        ...
        '''
        value, actor_features, states = self.base(observations, model_states, masks, cache=None)
        dist = self.dist(actor_features)
    ...
    def evaluate_actions(self, inputs, states, masks, action, cache=None):
        ''' TODO: find out what these do '''
        value, actor_features, states = self.base(inputs, states, masks, cache)
        dist = self.dist(actor_features)

        action_log_probs = dist.log_probs(action.to(self.gpu_devices[0]))
        dist_entropy = dist.entropy().mean()

        return value, action_log_probs, dist_entropy, states

    def compute_intrinsic_losses(self, loss_kwargs: dict, inputs, states, masks, action, cache: dict):
        losses = {}
        for intrinsic_loss in loss_kwargs['intrinsic_loss_types']:
            ...
            if intrinsic_loss == 'weight':
                pass
            if intrinsic_loss == 'transfer_l1':
                assert 'base_encoding' in cache and 'transfered_encoding' in cache,  f'cache does not contain base/transfered encoding. it contains {cache.keys()}'
                iloss = 0
                for base_encoding, transfered_encoding in zip(cache['base_encoding'], cache['transfered_encoding']):
                    iloss += self.l1(cache['net'].transfer(base_encoding), transfered_encoding)
                losses[intrinsic_loss] = iloss
            if intrinsic_loss == 'transfer_l2':
                assert 'base_encoding' in cache and 'transfered_encoding' in cache,  f'cache does not contain base/transfered encoding. it contains {cache.keys()}'
                iloss = 0
                for base_encoding, transfered_encoding in zip(cache['base_encoding'], cache['transfered_encoding']):
                    iloss += self.l2(cache['net'].transfer(base_encoding), transfered_encoding)
                losses[intrinsic_loss] = iloss
            if intrinsic_loss == 'perceptual_transfer':
                assert 'base_encoding' in cache and 'transfered_encoding' in cache,  f'cache does not contain base/transfered encoding. it contains {cache.keys()}'
                iloss = 0
                for base_encoding, transfered_encoding in zip(cache['base_encoding'], cache['transfered_encoding']):
                    base_decoding = self.loss_kwargs['decoder'](base_encoding)
                    transfered_decoding = self.loss_kwargs['decoder'](transfered_encoding)
                    iloss += self.l1(base_decoding, transfered_decoding)
                losses[intrinsic_loss] = iloss
            if intrinsic_loss == 'perceptual':
                assert 'base_encoding' in cache and 'merged_encoding' in cache,  f'cache does not contain base/merged encoding. it contains {cache.keys()}'
                iloss = 0
                for base_encoding, merged_encoding in zip(cache['base_encoding'], cache['merged_encoding']):
                    base_decoding = self.loss_kwargs['decoder'](base_encoding)
                    merged_decoding = self.loss_kwargs['decoder'](merged_encoding)
                    iloss += eval(self.loss_kwargs['loss_fn'])(base_decoding, merged_decoding)
                losses[intrinsic_loss] = iloss
        return losses

    def cuda(self, device=None):
        super().cuda(device)
        if 'decoder' in self.loss_kwargs:
            self.loss_kwargs['decoder'] = self.loss_kwargs['decoder'].to(device=device)

    def data_parallel(self, device_ids):
        self.base = _CustomDataParallel(self.base, device_ids)
        if 'decoder' in self.loss_kwargs:
            self.loss_kwargs['decoder'] = self.loss_kwargs['decoder'].to(device=device_ids[0])  # is this necessary?
            self.loss_kwargs['decoder'] = _CustomDataParallel(self.loss_kwargs['decoder'], device_ids)
